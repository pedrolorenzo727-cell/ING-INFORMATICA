...
Function BuscaBinaria(VecInt:TVInt; Pri,Ult:byte; X:integer):byte;
Var
  Medio:byte;
Begin
  Medio:=(Pri+Ult) DIV 2;
  If Pri<=Ult then
    If X<VecInt[Medio] then
      BuscaBinaria:=BuscaBinaria(VecInt,Pri,Medio-1,X)
    else
      If X>VecInt[Medio] then
        BuscaBinaria:=BuscaBinaria(VecInt,Medio+1,Ult,X)
      else
        BuscaBinaria:=Medio
  else
    BuscaBinaria:=0;
