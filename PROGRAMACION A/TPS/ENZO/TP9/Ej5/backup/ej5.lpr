  ...
  begin
    ...
    read(inscriptos, RI);
      ...
      if RA.matricula < RI.matricula then
        ...
        begin
             if RI.Q2 = 1 then
                writeln(RI.matricula, 'No se puede anotar a Quimica 2 porque no hay registro de sus notas':60);
             if RI.F2 = 1 then
                writeln(RI.matricula, 'No se puede anotar a Fisica 2 porque no hay registro de sus notas':60);
             read(inscriptos, RI);
        end;
  close(alumnos);
  close(inscriptos);
